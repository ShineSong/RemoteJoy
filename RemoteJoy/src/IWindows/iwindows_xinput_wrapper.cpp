    ...
    for (int i = 0; i < XUSER_MAX_COUNT; i++)
    {
        ...
        if (XInputGetStateEx(i, &xState) == ERROR_SUCCESS)
        {
            ...
            if (bSendLeftThumbstick) {
                TranslateSticks(i, xState.Gamepad.sThumbLX, xState.Gamepad.sThumbLY, X1_Left);
            }

            if (bSendRightThumbstick) {
                TranslateSticks(i, xState.Gamepad.sThumbRX, xState.Gamepad.sThumbRY, XI_Right);
            }

            emit StateRecieve(i, xState);
        }
    }
...
void IWindows_XInput_Wrapper::TranslateSticks(short uID, short X, short Y, IWindows_XInput_Enum e)
{
